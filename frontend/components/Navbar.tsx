...
import { LearnMenu, LEARN_LINKS } from "./LearnMenu";
        ...
        {LEARN_LINKS.map((link) => {
          ...
          return (
            <Link
              key={link.href}
              href={link.href}
              data-tour={link.href === "/judges" ? "nav-judges" : undefined}
              aria-current={active ? "page" : undefined}
              className={`whitespace-nowrap px-3 py-2 text-xs uppercase tracking-[0.14em] transition-colors ${
                active ? "text-foreground" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              {link.label === "Questions and answers"
                ? "FAQ"
                : link.label === "Verify in five minutes"
                  ? "Judges"
                  : link.label === "Developer docs"
                    ? "Docs"
                    : link.label}
            </Link>
          );
